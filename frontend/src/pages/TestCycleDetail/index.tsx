...
const TestCycleDetail: React.FC = () => {
  ...
  const [isConfirmDeleteSuiteOpen, setIsConfirmDeleteSuiteOpen] =
    useState(false);
  const [modalDeleteSuiteError, setModalDeleteSuiteError] = useState<
    string | null
  >(null);
  ...
  const fetchCicloDetails = useCallback(async () => {
    ...
    setError("");
    try {
      const cicloResponse = await fetch(
        `http://localhost:4000/ciclo-teste/${cicloId}`,
        {
      ...
      const projectResponse = await fetch(
        `http://localhost:4000/projeto/${data.id_projeto}`,
        {
          headers: { "user-id": user.id },
        }
      );
      if (!projectResponse.ok) {
        ...
        throw new Error(
          errData.error || "Falha ao verificar permissões do projeto."
        );
      }
      const projectData: { participantes: Participant[] } =
        await projectResponse.json();
      const manager = projectData.participantes.find(
        (p) => p.role.toLowerCase() === "gerente"
      );
  ...
  const handleBack = () => {
    ...
    if (targetProjectId) {
      navigate(`/home/projeto/${targetProjectId}`, {
        state: { defaultTab: "ciclo-teste" },
      });
    } else {
      navigate("/home");
    }
  ...
  const handleConfirmDelete = async (email: string, password: string) => {
    ...
    if (user?.email !== email) {
      return setModalError(
        "O e-mail digitado não corresponde ao da sua conta (gerente)."
      );
    }
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/ciclo-teste/${ciclo.id}`,
        {
          method: "DELETE",
      ...
      navigate(`/home/projeto/${ciclo.id_projeto}`, {
        state: { defaultTab: "ciclo-teste" },
      });
    } catch (err: any) {
  ...
  const handleConfirmDeleteSuite = async (email: string, password: string) => {
    ...
    if (user?.email !== email) {
      return setModalDeleteSuiteError(
        "O e-mail digitado não corresponde ao da sua conta (gerente)."
      );
    }
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/suite/${deletingSuite.id}`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json", "user-id": user.id },
          body: JSON.stringify({ email, password }),
  ...
  const itemsPool = ciclo.itens_gerais || [];
      ...
      <PageContainer>
        ...
        <Content>
          <DescriptionCard>
            ...
            <CardText>
              {ciclo.descricao || "Este ciclo não possui descrição."}
            </CardText>
          </DescriptionCard>

          <HeaderActions
            style={{ justifyContent: "flex-start", padding: "1rem 0" }}
          >
            <StartButton onClick={() => setIsCreateSuiteModalOpen(true)}>
              + Criar Nova Suíte
            </StartButton>
          </HeaderActions>
          ...
          <SuiteContainer>
            <SuiteHeader>
              <SuiteTitle>Itens do Ciclo de Teste</SuiteTitle>
            </SuiteHeader>

            {itemsPool.length === 0 ? (
              <p
                style={{ textAlign: "center", margin: "1rem 0", color: "#777" }}
              >
                Nenhum teste vinculado a este ciclo.
              </p>
            ) : (
              <TableResponsive>
                <BacklogTable>
          ...
          {ciclo.suites.map((suite) => (
            <SuiteContainer key={suite.id}>
              <SuiteHeader>
                ...
                <SuiteActions>
                  ...
                  <ViewReportButton
                    onClick={() =>
                      navigate(
                        `/home/ciclo-teste/${cicloId}/suite/${suite.id}/relatorio`
                      )
                    }
                  >
              ...
              {suite.descricao && (
                <CardText style={{ marginBottom: "1rem" }}>
                  {suite.descricao}
                </CardText>
              )}
